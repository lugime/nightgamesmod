...
public class XxxStore extends Store {
    ...
    public void visit(String choice) {
        ...
        if (player.human()) {
            ...
            GameState.gui().message("The adult specialty store stocks several items that could be useful during a match.");
            for (Item i : stock.keySet()) {
                if (!player.has(i)) {
                    GameState.gui().message(i.getName() + ": $" + i.getPrice());
                } else {
                    GameState.gui().message(i.getName() + ": $" + i.getPrice() + " (you have: "
                                    + player.getInventory().get(i) + ")");
                }
            }
            GameState.gui().message("You have :$" + player.money + " to spend.");
            sale(Item.Lubricant, GameState.gui());

            if (player.has(Item.Dildo)) {
                GameState.gui().message("You already have a perfectly serviceable dildo. You don't need another.");
            } else {
                sale(Item.Dildo, GameState.gui());
            }

            if (player.has(Item.Onahole)) {
                GameState.gui().message("You already have the best onahole in stock. You don't need another.");
            } else if (player.has(Item.Onahole2)) {
                GameState.gui().message("You already have the best onahole you can dream of.");
            } else {
                sale(Item.Onahole, GameState.gui());
            }

            if (player.has(Item.Crop)) {
                GameState.gui().message("You already have a riding crop. You don't need two.");
            } else if (player.has(Item.Crop2)) {
                GameState.gui().message("Your current riding crop is already overkill.");
            } else {
                sale(Item.Crop, GameState.gui());
            }

            if (player.has(Item.Tickler)) {
                GameState.gui().message("Your current tickler is at least as good as anything they are selling.");
            } else if (player.has(Item.Tickler2)) {
                GameState.gui().message("Nothing on sale is half as good as your current tickler.");
            } else {
                sale(Item.Tickler, GameState.gui());
            }

            if (player.hasDick()) {
                GameState.gui().message("You see a strap-on dildo for sale. It's no use to you since you have "
                                + "the real thing, but you should watch your ass if the girls start buying these.");
            } else if (player.has(Item.Strapon)) {
                GameState.gui().message("You are plenty satisfied with the strap-on you already have.");
            } else if (player.has(Item.Strapon2)) {
                GameState.gui().message("Your strapon is even better than the real thing already.");
            } else {
                sale(Item.Strapon, GameState.gui());
            }
            sale(Item.Blindfold, GameState.gui());
            displayClothes();
            if (Flag.checkFlag(Flag.AliceAvailable)) {
                ...
                choose("Talk to Alice", GameState.gui());
            }
            if (player.getLevel() >= 5 && !Flag.checkFlag(Flag.metAlice) && !Flag.checkFlag(Flag.victory)) {
                choose("Learn to be Submissive", GameState.gui());
            }
