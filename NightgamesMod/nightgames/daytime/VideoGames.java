...
public class VideoGames extends Activity {
    ...
    public void visit(String choice) {
        ...
        if (choice.equals("Start")) {
            if (player.money >= 50) {
                ...
                choose("Yes: $50", GameState.gui());
                choose("No", GameState.gui());
            } else {
                visit("No");
            }
            ...
            if (paid) {
                if (Random.random(3) == 0) {
                    GameState.gui().message("<br/><br/><b>You feel like your experiences have grown from playing the game.</b>");
    ...
    private void showScene(Scene chosen) {
        switch (chosen) {
            ...
            case mara1:
                ...
                player.gainAffection(CharacterPool.getNPC("Mara"), 1);
                CharacterPool.getNPC("Mara").gainAffection(player, 1);
            ...
            case cassiemara:
                ...
                CharacterPool.getNPC("Mara").gainAffection(CharacterPool.getNPC("Cassie"), 1);
                CharacterPool.getNPC("Cassie").gainAffection(CharacterPool.getNPC("Mara"), 1);
                break;
            case caroline1:
                GameState.gui().message(
    ...
    private Scene pickScene() {
        ...
        if (player.getAffection(CharacterPool.getNPC("Cassie")) >= 5) {
            available.add(Scene.cassie1);
        }
        if (player.getAffection(CharacterPool.getNPC("Cassie")) >= 10 && Flag.checkFlag(Flag.Cassievg1)) {
            available.add(Scene.cassie2);
        }
        if (player.getAffection(CharacterPool.getNPC("Mara")) >= 5) {
            available.add(Scene.mara1);
        }
        if (player.getAffection(CharacterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Maravg1)) {
            available.add(Scene.mara2);
        }
        if (CharacterPool.getNPC("Cassie").getAffection(CharacterPool.getNPC("Mara")) >= 10 && Flag.checkFlag(Flag.Cassievg2)
                        && Flag.checkFlag(Flag.maravg2)) {
            available.add(Scene.cassiemara);
        }
        if (player.getAffection(CharacterPool.getNPC("Angel")) >= 10) {
