            ...
            if (Flag.checkFlag(Flag.blackMarketPlus)) {
                if (!Flag.checkFlag(Flag.metRin)) {
                    ...
                    if (player.money >= 1000 * (player.getPure(Attribute.Dark) + 1)) {
                        ...
                        GameState.gui().message("You can't afford the artifacts.");
                    }
                    ...
                    if (player.money >= 1000 * (player.getPure(Attribute.Fetish) + 1)) {
                        ...
                        GameState.gui().message("You can't afford that.");
                    }
                } else {
                    GameState.gui().message(
                ...
                if (!trained) {
                    if (!Flag.checkFlag(Flag.darkness)) {
                        choose("Cursed Artifacts", GameState.gui());
                    } else {
                        choose("Dark Power: $" + 1000 * (player.getPure(Attribute.Dark) + 1), GameState.gui());
                    }
                    if (!Flag.checkFlag(Flag.fetishism)) {
                        choose("S&M Gear", GameState.gui());
                    } else {
                        choose("Fetishism: $" + 1000 * (player.getPure(Attribute.Fetish) + 1), GameState.gui());
                    }
                }
            } else {
                GameState.gui().message(
