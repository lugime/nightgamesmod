            ...
            if (Flag.checkFlag(Flag.blackMarketPlus)) {
                ...
                if (!trained) {
                    if (!Flag.checkFlag(Flag.darkness)) {
                        choose("Cursed Artifacts", GUI.gui);
                    } else {
                        choose("Dark Power: $" + 1000 * (player.getPure(Attribute.Dark) + 1), GUI.gui);
                    }
                    if (!Flag.checkFlag(Flag.fetishism)) {
                        choose("S&M Gear", GUI.gui);
                    } else {
                        choose("Fetishism: $" + 1000 * (player.getPure(Attribute.Fetish) + 1), GUI.gui);
                    }
                }
            } else {
                GUI.gui.message(
